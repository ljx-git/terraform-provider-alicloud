...
func manageUserPermissions(mode string, d *schema.ResourceData, meta interface{}, permissions []interface{}) error {
	...
	// call sdk describe cluster permissions for user
	perms, err := client.DescribeUserPermission(&uid)
	if err != nil {
		return WrapErrorf(err, DefaultErrorMsg, ResourceName, "DescribeUserPermission", AliyunTablestoreGoSdk)
	}
	addDebug("DescribeUserPermission", perms)
